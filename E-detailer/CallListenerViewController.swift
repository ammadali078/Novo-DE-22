...
class CallListenerViewController: UIViewController {
    ...
    func reloadData() {
        ...
        if data == "" {
            CommonUtils.showMsgDialog(showingPopupOn: self, withTitle: "Sante DCR", withMessage: "No Expense available to preview")
            return
        }
        if (data == "") {data = "[]"}
    ...
    @IBAction func onSubmitPressed(_ sender: Any) {
        ...
        //
        //                self.activitiyViewController.dismiss(animated: true, completion: {() in
        //                    CommonUtils.showMsgDialog(showingPopupOn: self, withTitle: "Sante Dcr", withMessage: "Sorry, Something went wrong")
        //                })
        //            }
        //
        //        })
    ...
    func ImageUpload() {
        ...
        let imageses = CommonUtils.getJsonFromUserDefaults(forKey: Constants.imgPostData)
        
        callImages = Mapper<ImageCallModel>().mapArray(JSONString: imageses)!
        
            if callImages.count == 0 {
                
                CommonUtils.showMsgDialog(showingPopupOn: self, withTitle: "Sante Dcr", withMessage: "No Expense available to sync")
                return
            }
                        ...
                        print(response.result.value)
                            ...
                            if loginModel != nil {
                                ...
                                if (loginModel?.success)! {
                                    ...
                                    CommonUtils.showMsgDialogWithOk(showingPopupOn: self, withTitle: "Novo", withMessage: "Email sent successfully", onOkClicked: {()
